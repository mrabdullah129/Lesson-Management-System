import { ArrowLeft, BadgeCheck, Code2, Laptop, Rocket } from 'lucide-react';
...
function DeveloperCard({ developer }) {
  ...
  return (
    <article className="group relative overflow-hidden rounded-[1.5rem] border border-white/10 bg-slate-950/60 p-5 text-white shadow-[0_25px_80px_rgba(0,0,0,0.35)] backdrop-blur-md transition duration-300 hover:-translate-y-1 hover:shadow-[0_30px_100px_rgba(0,0,0,0.45)] sm:rounded-[2rem] sm:p-6">
      ...
      <div className="relative z-10 flex h-full flex-col gap-5">
        ...
        <div className="space-y-3">
          <h3 className="text-3xl font-extrabold leading-tight tracking-tight text-white sm:text-4xl md:text-[3.35rem]">{developer.name}</h3>
          <p className="max-w-sm text-sm leading-7 text-slate-200 sm:text-base sm:leading-8">{developer.description}</p>
        </div>
        ...
        <div className="mt-auto">
          <a
            ...
            className="inline-flex w-full items-center justify-center rounded-2xl bg-white px-5 py-3.5 text-sm font-semibold text-slate-900 transition hover:bg-slate-100 sm:py-4 sm:text-base"
          >
            View Portfolio
          </a>
        </div>
...
function DeveloperPage() {
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,rgba(30,41,59,0.96),rgba(15,23,42,1)_55%,rgba(2,6,23,1)_100%)] px-3 py-5 text-white sm:px-4 sm:py-6 md:px-8 lg:px-10">
      <div className="mx-auto max-w-7xl">
        <div className="mb-6 flex flex-col items-start justify-between gap-3 sm:mb-8 sm:flex-row sm:items-center sm:gap-4">
          ...
          <span className="rounded-full border border-white/15 bg-white/10 px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.2em] text-white/70 sm:px-4 sm:text-xs sm:tracking-[0.25em]">
            Meet the developer
          </span>
        </div>

        <div className="mb-6 sm:mb-8">
          <h1 className="max-w-4xl text-3xl font-black tracking-tight text-white sm:text-4xl md:text-6xl">The talented minds building this system</h1>
        </div>
        ...
        <div className="mt-6 grid gap-4 rounded-[1.5rem] border border-white/10 bg-white/5 p-4 backdrop-blur-md sm:mt-8 sm:rounded-[2rem] sm:p-6 lg:grid-cols-3">
